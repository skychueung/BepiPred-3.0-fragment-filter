...
def top_n_per_parent(df: pd.DataFrame, n: int) -> pd.DataFrame:
    if df.empty or n is None:
        return df
    sort_cols = ["parent_rank", "heuristic_rank_score", "fragment_length", "offset_in_parent"]
    ascending = [True, False, False, True]
    ranked = df.sort_values(sort_cols, ascending=ascending).copy()
    top = ranked.groupby("parent_rank", as_index=False).head(n).copy()
    top = top.sort_values(["parent_rank", "fragment_start", "fragment_end"]).reset_index(drop=True)
    return top
...
def run_fragment_filter(
    ...
    if top_n is not None:
        result = top_n_per_parent(result, top_n)

    if output_name is None:
        suffix = "tsv"
        name_parts = [f"fragment_candidates_{mode}", str(min_len), str(max_len)]
        if top_n is not None:
            name_parts.append(f"top{top_n}")
        output_name = "_".join(name_parts) + f".{suffix}"
